...
class MockAdlaService:
    ...
    def submit_job(self, account, job_submission):
        return AdlaJob(str(uuid4()), job_submission.name, "USql", "mock_submitter", job_submission.parallelism, job_submission.priority, datetime.now(), datetime.now(), datetime.now(), "mock_state", "mock_result")

    def retrieve_job(self, account, job_id):
        return AdlaJob(job_id, "mock_job_name", "USql", "mock_submitter", 5, 100, datetime.now(), datetime.now(), datetime.now(), "mock_state", "mock_result")
